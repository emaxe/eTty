...
**REQ-3.** В редакторе (CodeMirror 6) отображается вертикальная цветная полоска (3px) в gutter слева от номеров строк (расширение `diffGutterExtension` объявлено перед `lineNumbers()`, поэтому рендерится левее их):
- Добавленные строки — зелёная полоска (`--green`)
- Изменённые строки (хank содержит и `+` и `-`) — синяя полоска (`--accent`)
- Позиции чистых удалений (без соседнего `+`) — с v2 отображаются красным треугольным маркером (`.diff-gutter-deleted`) на строке, следующей сразу после удаления; если удаление в самом конце файла — маркер прижимается к последней строке
...
- Не добавлять новые IPC-каналы: использовать существующие `git:get-status` и `git:get-diff` (в v2 `git:get-diff` для этого use case вызывается с `{ compareTo: 'head' }`, чтобы сохранить комбинированный staged+unstaged diff относительно HEAD — редактор не отслеживает staged-статус per-file)
